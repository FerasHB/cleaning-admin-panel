// lib/timesheets/timesheetHtml.ts
// Baut das druckbare HTML für den Arbeitszeitnachweis (Stundenzettel).
// Port von Mobiles services/timesheets/timesheetHtml.ts — identische Felder,
// identisches Tabellenlayout, identischer Inhalt. Bewusst eigenes, neutrales
// Layout — KEIN DATEV-Branding, kein Logo, keine fremden Marken.
//
// WEB-SPEZIFISCH: Mobile erzeugt daraus über expo-print eine PDF-Datei und
// teilt sie (expo-sharing) — beides React-Native-only, auf Web nicht
// verfügbar. Web nutzt stattdessen den browsereigenen Druckdialog
// (window.print() über ein neues Fenster mit genau diesem HTML) — "Als PDF
// speichern" ist dort ein Standard-Ziel jedes Browsers. Keine neue
// Abhängigkeit nötig; Inhalt/Ergebnis ist identisch zu Mobile.

import type { TimesheetData, TimesheetEntry } from "@/lib/timesheets/timesheet.service"
...
export function buildTimesheetHtml(data: TimesheetData): string {
  const rows = data.entries.length > 0 ? data.entries.map(renderRow).join("") : renderEmptyRow()
...
<title>Arbeitszeitnachweis ${escapeHtml(data.employeeName)} ${escapeHtml(data.monthLabel)}</title>
<style>
...
/**
 * Öffnet den Stundenzettel als druckbares HTML in einem neuen Tab und
 * startet sofort den Druckdialog — "Als PDF speichern" ist dort ein
 * Standard-Ziel jedes Browsers. Kein Datei-Download, keine neue Abhängigkeit.
 * Wirft, wenn Pop-ups blockiert sind (Aufrufer zeigt dann eine Fehlermeldung).
 */
export function exportTimesheetPdf(data: TimesheetData): void {
  const html = buildTimesheetHtml(data)
  const printWindow = window.open("", "_blank")
  if (!printWindow) {
    throw new Error(
      "Der Druckdialog konnte nicht geöffnet werden. Bitte Pop-ups für diese Seite erlauben.",
    )
  }
  ...
  printWindow.focus()
  // Warten, bis das neue Dokument vollständig gerendert ist, bevor gedruckt
  // wird — ein sofortiger print() auf ein noch leeres Dokument zeigt in
